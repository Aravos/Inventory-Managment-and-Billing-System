...
connection = mysql.connector.connect(user='root', password='mysql', host='localhost')
...
def total():
    ...
    connection.commit()
    cursor.execute("select transaction.Bill_number,SUM(transaction.Quantity*product.price) from transaction inner join product on transaction.product_ID = product.product_ID where bill_number="+l[0][0]+" Group by Bill_number;")
    ...
    # billno = l[0][0]
    l.clear()
    return render_template('total.html',data=result)
